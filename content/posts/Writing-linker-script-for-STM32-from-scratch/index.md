...
第二个很可能就是 main 函数的地址 `0x08000009`，最后一位表示 thumb 指令集，实际应该是 `0x08000008`
...
最后下载程序，成功点亮 led，我用的是 stlink 烧录器，使用 st-flash 命令烧录（见 makefile）：
...
如果你写入程序，会发现 led 不会点亮，因为 `a` 的值不会自己从 `0x08000048` 跑到 `0x20000000` 地址处， 我们需要编写代码完成这个操作。
...
> 类似的，初始化 `.bss` 只需要将 `.bss` 在栈上的区域赋值为 0 即可。
